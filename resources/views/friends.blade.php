   ...
   <x-dashboard-sidebar />
    <main class="flex-1 h-screen overflow-y-auto p-6 hide-scrollbar">
        ...
        <div id="view-grid" class="view-container view-active max-w-6xl mx-auto">
            <div id="friends-grid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4"></div>
        </div>
        ...
        <div id="view-table" class="view-container max-w-6xl mx-auto">
            <div class="bg-white border border-slate-200 rounded-custom overflow-hidden">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody id="friends-table-body"></tbody>
                </table>
            </div>
        </div>

        <div id="view-line" class="view-container max-w-6xl mx-auto">
            <div id="friends-line" class="space-y-3"></div>
        </div>
    ...
    <script>
        ...
        const friendsLine = document.getElementById('friends-line');
        let friendsData = [];
        ...
        function renderFriends(list) {
            friendsGrid.innerHTML = list.map((user) => `
            ...
            friendsTableBody.innerHTML = list.map((user) => `
                <tr class="border-b border-slate-100 last:border-0 transition-colors">
                    ...
                    <td class="px-6 py-4"><span class="px-2 py-1 text-[10px] font-bold rounded-custom bg-blue-50 text-blue-600">Friend</span></td>
                    <td class="px-6 py-4 text-right"><a href="${user.profileUrl}" class="text-blue-600 font-bold text-xs">Profile</a></td>
                </tr>
            `).join('');

            friendsLine.innerHTML = list.map((user) => `
                <div class="bg-white border border-slate-200 p-3 rounded-custom flex items-center justify-between group hover:border-blue-400 transition-colors">
                    ...
                    <span class="text-xs text-slate-400 italic">Following each other</span>
                </div>
            `).join('');
        }

        async function loadFriends() {
            const response = await fetch('/api/friends', { headers: { Accept: 'application/json' } });
